...
type SortKey = "maxDailyProfit" | "profit" | "marginPercent" | "salePriceGross" | "totalCost";
type MarketFilter = "all" | "bazaar" | "ah-lowest-bin";
type MayorMode = "auto" | "normal" | "diaz";
...
function strategyLabel(flip: NpcFlip): string {
  if (flip.maxProfitStrategy === "sell-order") return "BZ Sell Order";
  if (flip.maxProfitStrategy === "insta-sell") return "BZ Insta Sell";
  return "AH 稅後估價";
}
...
export function NpcFlipDashboard() {
  ...
  const [mayorMode, setMayorMode] = useState<MayorMode>("auto");
  ...
  const diazActive = mayorMode === "diaz" || (mayorMode === "auto" && data.mayor.shoppingSpreeActive);
  ...
  return <>
    <div className="toolbar panel npc-flip-toolbar">
      ...
      <label><span>排序</span><select value={sort} onChange={(event) => setSort(event.target.value as SortKey)}>
        ...
        <option value="marginPercent">Margin (%)</option><option value="salePriceGross">出售價格</option><option value="totalCost">單次成本</option>
      </select></label>
      ...
      <label><span>市長上限</span><select value={mayorMode} onChange={(event) => setMayorMode(event.target.value as MayorMode)}>
        <option value="auto">自動：{data.mayor.name}{data.mayor.shoppingSpreeActive ? " ×10" : " ×1"}</option>
        <option value="normal">一般上限（×1）</option><option value="diaz">Diaz Shopping Spree（×10）</option>
      </select></label>
    ...
    {loading && !hasLoadedRef.current
      ...
      : error && !hasLoadedRef.current
        ...
        : <div className="market-table-wrap panel"><table className="market-table npc-flip-table"><thead><tr>
          <th>NPC 商品</th><th>NPC</th><th>單次購買需求</th><th>單次成本</th><th>出售價格</th><th>7日成交量</th><th>單次 Profit</th><th>每日上限</th><th>Max Profit</th>
        </tr></thead><tbody>{displayedFlips.slice(0, 300).map((flip) => {
          const plan = planFor(flip);
          return <tr key={flip.offerId}>
            <td><button className="npc-item-button" type="button" onClick={() => setSelectedOfferId(flip.offerId)}><span className="item-cell"><ItemIcon name={flip.name} productId={flip.productId} /><span><strong>{flip.quantity > 1 ? `${flip.quantity}× ` : ""}{flip.name}</strong><small>{flip.productId}</small></span></span></button></td>
            <td><span className="stack"><strong>{flip.npc}</strong>{flip.requirement ? <small>{flip.requirement}</small> : null}</span></td>
            <td><CostList costs={flip.costs.map((cost) => ({ name: cost.name, amount: cost.amount, totalPrice: cost.totalPrice, priceSource: cost.priceSource }))} /></td>
            <td>{formatCoins(flip.totalCost)}</td>
            <td><SalePrice flip={flip} /></td>
            ...
            <td><span className={`stack ${tone(flip.maxProfitPerPurchase)}`}><strong>{formatCoins(flip.maxProfitPerPurchase)}</strong><small>{strategyLabel(flip)} · {formatPercent(flip.totalCost > 0 ? flip.maxProfitPerPurchase / flip.totalCost * 100 : 0)}</small>{flip.saleSource === "bazaar" ? <small>Insta {formatCoins(flip.profit)} · Order {formatCoins(flip.bazaarSellOrderProfit ?? 0)}</small> : null}</span></td>
            <td>{plan ? <span className="stack"><strong>{integer(plan.purchaseCount)} 次 · {integer(plan.outputQuantity)} 個</strong><small>{plan.diazApplied ? "Diaz ×10 已套用" : flip.diazEligible ? "一般每日上限" : "特殊上限（不吃 Diaz）"}{plan.conditionalBonusApplied ? " · Contact +1" : ""}</small></span> : <span className="stack neutral"><strong>未確認</strong><small>沒有可靠的每日庫存資料</small></span>}</td>
            <td>{plan ? <span className="stack npc-max-profit"><strong className={tone(plan.totalProfit)}>{formatCoins(plan.totalProfit)}</strong><CostList costs={plan.costs.map((cost) => ({ name: cost.name, amount: cost.requiredAmount, totalPrice: cost.totalPrice, priceSource: cost.priceSource }))} compact /><button className="detail-button" type="button" onClick={() => setSelectedOfferId(flip.offerId)}>查看詳細</button></span> : <span className="stack"><strong>無法估算</strong><small>{formatCoins(flip.maxProfitPerPurchase)} / 次</small><button className="detail-button" type="button" onClick={() => setSelectedOfferId(flip.offerId)}>查看詳細</button></span>}</td>
          </tr>;
        })}</tbody></table>{displayedFlips.length === 0 ? <div className="empty-state">目前沒有符合條件且可完整定價的 NPC Flip。</div> : null}</div>}
    {selectedFlip ? <NpcFlipDetailModal flip={selectedFlip} diazActive={diazActive} conditionalBonusActive={conditionalBonusActive} onConditionalBonusChange={setConditionalBonusActive} onClose={() => setSelectedOfferId(null)} mayorLabel={mayorMode === "auto" ? `自動：${data.mayor.name}` : mayorMode === "diaz" ? "Diaz Shopping Spree" : "一般上限"} /> : null}
    <p className="npc-disclaimer">Max Profit 會在 BZ Insta Sell 與 Sell Order 中選擇稅後利潤較高者，並乘上可靠的每日可購買次數。一般可轉售的 NPC 商品採 640 個標準上限；Diaz Shopping Spree 適用時乘 10。Kiara 的 Shard 為特殊庫存且不套用 Diaz，解鎖 Abiphone Contact 後各增加 1 個。AH 仍使用最低 BIN／近期成交中位估價並扣除分級手續費。市場價格由 Hypixel 與 <a href="https://sky.coflnet.com/data" target="_blank" rel="noreferrer">SkyCofl</a> 提供。</p>
  </>;
}

function SalePrice({ flip }: { flip: NpcFlip }) {
  return <span className="stack"><strong>{formatCoins(flip.salePriceGross)}</strong>{flip.saleSource === "bazaar"
    ? <small><span className="market-source-badge">{flip.bazaarInstaSellAvailable === false ? "BZ sell order" : "BZ insta sell"}</span> · 稅後 {formatCoins(flip.salePriceNet)}</small>
...
function CostList({ costs, compact = false }: {
  costs: Array<{ name: string; amount: number; totalPrice: number; priceSource: "coins" | "bazaar" | "ah-lowest-bin" }>;
  compact?: boolean;
}) {
  return <span className={`stack npc-cost-list${compact ? " compact" : ""}`}>{costs.map((cost, index) => <span key={`${cost.name}-${index}`}><strong>{integer(cost.amount)}× {cost.name}</strong>{compact ? null : <small>{cost.priceSource === "coins" ? formatCoins(cost.totalPrice) : `${cost.priceSource === "bazaar" ? "BZ insta buy" : "AH LBIN"} · ${formatCoins(cost.totalPrice)}`}</small>}</span>)}</span>;
}
...
function NpcFlipDetailModal({ flip, diazActive, conditionalBonusActive, onConditionalBonusChange, mayorLabel, onClose }: {
  ...
  return <div className="detail-modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><section className="shard-detail-modal npc-detail-modal panel" role="dialog" aria-modal="true" aria-labelledby="npc-detail-title">
    <header><div><span className="eyebrow">NPC Max Profit detail</span><h2 id="npc-detail-title">{flip.name}</h2><code>{flip.npc} · {flip.productId}</code></div><button type="button" aria-label="關閉" onClick={onClose}>×</button></header>
    {plan ? <><div className="detail-profit-grid"><div><span>需要購買</span><strong>{integer(plan.purchaseCount)} 次</strong></div><div><span>取得成品</span><strong>{integer(plan.outputQuantity)} 個</strong></div><div><span>成本總額</span><strong>{formatCoins(plan.totalCost)}</strong></div><div><span>{fraction === 1 ? "Max Profit" : "80% Target Profit"}</span><strong className={tone(plan.totalProfit)}>{formatCoins(plan.totalProfit)}</strong></div></div>
      <div className="route-multiplier npc-plan-controls"><label><span>成本規劃目標</span><select value={fraction} onChange={(event) => setFraction(Number(event.target.value) as 1 | 0.8)}><option value={1}>拿完每日庫存（100% Max Profit）</option><option value={0.8}>Max Profit 的至少 80%</option></select></label><div><span>市長模式</span><strong>{mayorLabel}{plan.diazApplied ? " · ×10" : " · ×1"}</strong></div><div><span>採用出售方式</span><strong>{strategyLabel(flip)}</strong></div><div><span>稅後總收入</span><strong>{formatCoins(plan.revenueAfterTax)}</strong></div>{flip.conditionalDailyLimitBonus ? <label className="npc-modal-checkbox"><span>{flip.conditionalLimitRequirement}</span><span><input type="checkbox" checked={conditionalBonusActive} onChange={(event) => onConditionalBonusChange(event.target.checked)} /> 套用庫存 +{flip.conditionalDailyLimitBonus}</span></label> : null}</div>
      <div className="detail-columns"><article><div className="modal-section-title"><div><span className="eyebrow">Required costs</span><h3>{fraction === 1 ? "拿完所有庫存所需成本" : "達到 80% Max Profit 所需成本"}</h3></div><small>數量已向上取整到完整購買次數</small></div><div className="material-total-list">{plan.costs.map((cost) => <div key={cost.productId ?? cost.name}><span><strong>{cost.name}</strong><code>{cost.productId ?? "COINS"} · 每次 {integer(cost.amountPerPurchase)}</code></span><span><strong>{integer(cost.requiredAmount)} 個</strong><small>{formatCoins(cost.totalPrice)} · {cost.priceSource === "bazaar" ? "BZ insta buy" : cost.priceSource === "ah-lowest-bin" ? "AH LBIN" : "Coins"}</small></span></div>)}</div></article>
        <article><div className="modal-section-title"><div><span className="eyebrow">Limit and revenue audit</span><h3>上限與收益</h3></div></div><div className="material-total-list"><div><span><strong>基礎每日庫存</strong><small>{limitSourceLabel(flip.dailyLimitSource)}</small></span><span><strong>{integer(flip.dailyLimit ?? 0)} 個</strong><small>{flip.diazEligible ? "可套用 Diaz" : "不適用 Diaz"}</small></span></div>{flip.conditionalDailyLimitBonus ? <div><span><strong>條件庫存</strong><small>{flip.conditionalLimitRequirement}</small></span><span><strong>+{flip.conditionalDailyLimitBonus}</strong><small>{plan.conditionalBonusApplied ? "已套用" : "未套用"}</small></span></div> : null}<div><span><strong>有效每日庫存</strong><small>{plan.purchaseCount} 次購買</small></span><span><strong>{integer(plan.effectiveDailyLimit)} 個</strong><small>{plan.diazApplied ? "Shopping Spree ×10" : "×1"}</small></span></div><div><span><strong>單次最佳 Profit</strong><small>{strategyLabel(flip)}</small></span><span><strong className={tone(flip.maxProfitPerPurchase)}>{formatCoins(flip.maxProfitPerPurchase)}</strong><small>成本 {formatCoins(flip.totalCost)}</small></span></div></div><p className="npc-detail-source">資料來源：<a className="source-link" href={flip.source.url} target="_blank" rel="noreferrer">{flip.source.label}</a></p></article></div></>
    : <div className="empty-state">這筆商品沒有足夠可靠的每日庫存資料，因此不虛構 Max Profit；仍可使用單次 Profit 判斷。</div>}
    <footer><span>每日上限依商品數量計算；組合包會換算為可完整購買的次數。</span><span>近 7 天成交量不等於保證能立即成交。</span></footer>
  </section></div>;
}
