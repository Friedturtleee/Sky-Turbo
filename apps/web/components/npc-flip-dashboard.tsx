...
export function NpcFlipDashboard() {
  ...
  return <>
      ...
      : error && !hasLoadedRef.current
        ...
        : <div className="market-table-wrap panel npc-table-panel"><table className="market-table npc-flip-table"><thead><tr>
          ...
          return <tr key={flip.offerId}>
            ...
            <td data-label="每日上限">{plan ? <span className="stack"><strong>{integer(plan.purchaseCount)} 次 · {integer(plan.outputQuantity)} 個</strong><small>{plan.diazApplied ? "Diaz ×10 已自動套用" : flip.diazEligible ? "一般每日上限" : "特殊上限（不吃 Diaz）"}{plan.conditionalBonusApplied ? " · Contact +1" : ""}</small></span> : <span className="stack neutral"><strong>未確認</strong><small>沒有可靠的每日庫存資料</small></span>}</td>
    ...
    <p className="npc-disclaimer">NPC Flip 依目前選取的 Buy Order／Instant Buy 與 Sell Order／Instant Sell 策略計算。一般可轉售的 NPC 商品採 640 個標準上限；現任 Mayor 與 Diaz Shopping Spree 由 Hypixel Election API 自動套用。Kiara 的 Shard 為特殊庫存且不套用 Diaz，解鎖 Abiphone Contact 後各增加 1 個。AH 仍使用最低 BIN／近期成交中位估價並扣除分級手續費。市場價格由 Hypixel 與 <a href="https://sky.coflnet.com/data" target="_blank" rel="noreferrer">SkyCofl</a> 提供。</p>
  </>;
}
...
function SalePrice({ flip }: { flip: NpcFlip }) {
  return <span className="stack"><strong>{formatCoins(flip.salePriceGross)}</strong>{flip.saleSource === "bazaar"
    ? <small><span className="market-source-badge">{flip.maxProfitStrategy === "sell-order" ? "BZ sell order" : "BZ insta sell"}</span> · 稅後 {formatCoins(flip.salePriceNet)}</small>
...
function NpcFlipDetailModal({ flip, diazActive, conditionalBonusActive, onConditionalBonusChange, mayorLabel, onClose }: {
  ...
  return <div className="detail-modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><section className="shard-detail-modal npc-detail-modal panel" role="dialog" aria-modal="true" aria-labelledby="npc-detail-title">
    ...
    {plan ? <><div className="detail-profit-grid"><div><span>需要購買</span><strong>{integer(plan.purchaseCount)} 次</strong></div><div><span>取得成品</span><strong>{integer(plan.outputQuantity)} 個</strong></div><div><span>成本總額</span><strong>{formatCoins(plan.totalCost)}</strong></div><div><span>{fraction === 1 ? "Max Profit" : "80% Target Profit"}</span><strong className={tone(plan.totalProfit)}>{formatCoins(plan.totalProfit)}</strong></div></div>
      <div className="route-multiplier npc-plan-controls"><label><span>成本規劃目標</span><select value={fraction} onChange={(event) => setFraction(Number(event.target.value) as 1 | 0.8)}><option value={1}>拿完每日庫存（100% Max Profit）</option><option value={0.8}>Max Profit 的至少 80%</option></select></label><div><span>市長（自動）</span><strong>{mayorLabel}{plan.diazApplied ? " · ×10" : " · ×1"}</strong></div><div><span>交易策略</span><strong>{strategyLabel(flip)}</strong></div><div><span>稅後總收入</span><strong>{formatCoins(plan.revenueAfterTax)}</strong></div>{flip.conditionalDailyLimitBonus ? <label className="npc-modal-checkbox"><span>{flip.conditionalLimitRequirement}</span><span><input type="checkbox" checked={conditionalBonusActive} onChange={(event) => onConditionalBonusChange(event.target.checked)} /> 套用庫存 +{flip.conditionalDailyLimitBonus}</span></label> : null}</div>
      <div className="detail-columns"><article><div className="modal-section-title"><div><span className="eyebrow">Required costs</span><h3>{fraction === 1 ? "拿完所有庫存所需成本" : "達到 80% Max Profit 所需成本"}</h3></div><small>數量已向上取整到完整購買次數</small></div><div className="material-total-list">{plan.costs.map((cost) => <div key={cost.productId ?? cost.name}><span><strong>{cost.name}</strong><code>{cost.productId ? productReference(cost.productId) : "COINS"} · 每次 {integer(cost.amountPerPurchase)}</code></span><span><strong>{integer(cost.requiredAmount)} 個</strong><small>{formatCoins(cost.totalPrice)} · {cost.priceSource === "bazaar" ? bazaarCostLabel(flip.strategy) : cost.priceSource === "ah-lowest-bin" ? "AH LBIN" : "Coins"}</small></span></div>)}</div></article>
        <article><div className="modal-section-title"><div><span className="eyebrow">Limit and revenue audit</span><h3>上限與收益</h3></div></div><div className="material-total-list"><div><span><strong>基礎每日庫存</strong><small>{limitSourceLabel(flip.dailyLimitSource)}</small></span><span><strong>{integer(flip.dailyLimit ?? 0)} 個</strong><small>{flip.diazEligible ? "可套用 Diaz" : "不適用 Diaz"}</small></span></div>{flip.conditionalDailyLimitBonus ? <div><span><strong>條件庫存</strong><small>{flip.conditionalLimitRequirement}</small></span><span><strong>+{flip.conditionalDailyLimitBonus}</strong><small>{plan.conditionalBonusApplied ? "已套用" : "未套用"}</small></span></div> : null}<div><span><strong>有效每日庫存</strong><small>{plan.purchaseCount} 次購買</small></span><span><strong>{integer(plan.effectiveDailyLimit)} 個</strong><small>{plan.diazApplied ? "Shopping Spree ×10" : "×1"}</small></span></div><div><span><strong>單次 Profit</strong><small>{strategyLabel(flip)}</small></span><span><strong className={tone(flip.maxProfitPerPurchase)}>{formatCoins(flip.maxProfitPerPurchase)}</strong><small>成本 {formatCoins(flip.totalCost)}</small></span></div></div><p className="npc-detail-source">資料來源：<a className="source-link" href={flip.source.url} target="_blank" rel="noreferrer">{flip.source.label}</a></p></article></div></>
