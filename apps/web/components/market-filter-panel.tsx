...
import type { MarketFilterKey } from "@sky-turbo/core";
...
export function appendMarketFilters(query: URLSearchParams, filters: MarketFilterDrafts): void {
  for (const key of keys) {
    if (filters[key].min !== "") query.set(`${key}Min`, filters[key].min);
    if (filters[key].max !== "") query.set(`${key}Max`, filters[key].max);
  }
}
...
export function MarketFilterPanel({
  ...
  return <details className="filters"><summary>{summary}</summary><div className="filter-grid">
    ...
    {keys.map((key) => <fieldset key={key}><legend>{labels[key]}</legend>
      <input type="number" placeholder="最小" value={drafts[key].min} onChange={(event) => setDrafts((current) => ({ ...current, [key]: { ...current[key], min: event.target.value } }))} />
      <input type="number" placeholder="最大" value={drafts[key].max} onChange={(event) => setDrafts((current) => ({ ...current, [key]: { ...current[key], max: event.target.value } }))} />
    </fieldset>)}
